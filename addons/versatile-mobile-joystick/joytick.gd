...
func _update_input_actions(output: Vector2) -> void:
	...
	for action in actions:
		var iev = InputEventAction.new()
		iev.action = action.action
		iev.pressed = action.condition
		iev.strength = action.strength
		Input.parse_input_event(iev)
		#if action.condition:
			#Input.action_press(action.action, action.strength)
			#print(action.action,  action.condition, action.strength)
		#else:
